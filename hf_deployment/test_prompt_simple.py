...
def test_few_shot_examples():
    ...
    print(f"Query: {query}")
    print(f"Few-shot examples available: {len(template.few_shot_examples) if template.few_shot_examples else 0}")
    ...
    print(f"Length increase: {len(prompt_with_fs['user']) - len(prompt_no_fs['user'])} chars")
...
def test_chain_of_thought():
    ...
    try:
        ...
        print(f"  Has reasoning framework: {'REASONING PROCESS:' in cot_prompt['user']}")
        
        # Show reasoning structure
        if "REASONING PROCESS:" in cot_prompt['user']:
            reasoning_section = cot_prompt['user'].split("REASONING PROCESS:")[1].split("STRUCTURED REASONING:")[0]
