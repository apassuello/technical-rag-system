...
class ConfigValidator:
    ...
    def validate_all_configs(self, run_end_to_end: bool = True) -> Dict[str, Dict[str, Any]]:
        """
        ...
        for config_file in config_files:
            ...
            results[config_name] = self._validate_config_file(config_file, run_end_to_end)
            
        return results
    
    def _validate_config_file(self, config_path: Path, run_end_to_end: bool = True) -> Dict[str, Any]:
        """
    ...
    def _validate_components(self, config: PipelineConfig, result: Dict[str, Any]) -> None:
    ...
    def _validate_component_config(self, comp_name: str, comp_config, result: Dict[str, Any]) -> None:
        ...
        if comp_name == "embedder":
            ...
            if "embedding_dim" in config_dict:
                ...
                if "embedding_dim" in vs_config and vs_config["embedding_dim"] != emb_dim:
            ...
            # Check for API token if using HuggingFace models
            model_name = config_dict.get("model_name", "")
            if "huggingface" in model_name.lower() or "/" in model_name:
                api_token = config_dict.get("api_token")
                if not api_token and not config_dict.get("use_ollama", False):
                    comp_result["issues"].append(
                        "HuggingFace model specified but no API token provided and Ollama not enabled"
                    )
        
        # Add any issues to main errors list
        if comp_result["issues"]:
            comp_result["config_valid"] = False
            result["errors"].extend([f"{comp_name}: {issue}" for issue in comp_result["issues"]])
    
    def _test_pipeline_initialization(self, config_path: Path, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            required_components = ["processor", "embedder", "vector_store", "retriever", "generator"]
    ...
    def _add_recommendations(self, config: PipelineConfig, result: Dict[str, Any]) -> None:
    ...
    def _test_document_processing(self, pipeline: RAGPipeline) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ Document processing: {chunk_count} chunks in {processing_time:.2f}s")
    ...
    def _test_query_execution(self, pipeline: RAGPipeline) -> Dict[str, Any]:
        ...
        try:
            for i, query in enumerate(test_queries):
                ...
                logger.info(f"✅ Query completed in {query_time:.3f}s, confidence: {answer.confidence:.3f}")
            
            result.update({
                "success": True,
                "successful_queries": successful_queries,
                "average_query_time": total_time / successful_queries if successful_queries > 0 else 0,
                "average_confidence": total_confidence / successful_queries if successful_queries > 0 else 0,
    ...
    def _benchmark_performance(self, pipeline: RAGPipeline) -> Dict[str, Any]:
        ...
        try:
            ...
            retrieval_times = []
            test_queries = ["RISC-V", "instruction", "register", "encoding", "architecture"]
            ...
            logger.info(f"📊 Performance: {memory_mb:.1f}MB memory, {result['retrieval_speed']['average_ms']:.1f}ms avg retrieval")
            ...
            if "end_to_end_results" in result and result["end_to_end_results"]:
                ...
                if "document_processing" in e2e:
                    ...
                    if doc_result.get("success", False):
                        ...
                        print(f"   📄 Document processing: ✅ {chunks} chunks in {time_taken:.2f}s")
                    else:
                        print(f"   📄 Document processing: ❌ {doc_result.get('error', 'Failed')}")
                ...
                if "query_execution" in e2e:
                    ...
                    if query_result.get("success", False):
                        ...
                        print(f"   🔍 Query execution: ✅ {queries}/5 queries, {avg_time:.3f}s avg, {avg_conf:.3f} conf")
                    else:
                        print(f"   🔍 Query execution: ❌ {query_result.get('error', 'Failed')}")
                ...
                if "performance_metrics" in result and result["performance_metrics"]:
                    ...
                    if "memory_usage_mb" in perf and "retrieval_speed" in perf:
                        ...
                        print(f"   📊 Performance: {memory:.1f}MB memory, {retrieval_ms:.1f}ms retrieval")
    ...
    def save_detailed_report(self, results: Dict[str, Dict[str, Any]], output_path: Path) -> None:
        ...
        report = {
            ...
            "summary": {
                "total_configs": len(results),
                "valid_configs": sum(1 for r in results.values() if r["valid"]),
                "success_rate": sum(1 for r in results.values() if r["valid"]) / len(results) * 100
            },
            "results": results
        }
...
def main():
    ...
    parser.add_argument("--config", type=str, 
                       help="Test specific configuration file")
    ...
    if args.config:
        ...
        results = {args.config: validator._validate_config_file(config_path, not args.basic)}
    else:
