...
class ModularUnifiedRetriever(Retriever):
    ...
    def __init__(self, config: Dict[str, Any], embedder: Embedder):
        ...
        self.max_candidates_multiplier = (
            composite_config.get("max_candidates", 15) / 10.0
        )  # Convert to multiplier (1.5x)
    ...
    def _create_fusion_strategy(self, config: Dict[str, Any]) -> FusionStrategy:
        ...
        if fusion_type == "rrf":
            ...
            raise ValueError(
                f"Unknown fusion strategy type: {fusion_type}. Available options: rrf, weighted, graph_enhanced_rrf, score_aware"
            )
    ...
    def _create_reranker(self, config: Dict[str, Any]) -> Reranker:
        ...
        logger.info(
            f"🔧 Creating reranker: type={reranker_type}, config keys={list(reranker_config.keys())}"
        )
        ...
        if reranker_type == "semantic":
            ...
            try:
                ...
                logger.info(
                    f"✅ NeuralReranker created successfully: enabled={reranker.enabled}, initialized={reranker._initialized}"
                )
    ...
    def retrieve(self, query: str, k: int = 5) -> List[RetrievalResult]:
        ...
        try:
            ...
            logger.info(
                f"🔍 MODULAR RETRIEVER: Starting retrieval for query: '{query}' (k={k})"
            )
            ...
            logger.info(
                f"🔤 QUERY EMBEDDING: Generated {query_embedding.shape} dimensional vector"
            )

            # Step 2: Dense vector search (with efficiency optimization)
            candidate_multiplier = (
                int(self.max_candidates_multiplier * k)
                if self.composite_filtering_enabled
                else k * 2
            )
            logger.info(
                f"🎯 DENSE SEARCH: Searching for top {candidate_multiplier} candidates"
            )
            dense_results = self.vector_index.search(
                query_embedding, k=candidate_multiplier
            )
            logger.info(f"✅ DENSE RESULTS: {len(dense_results)} documents found")

            # Log top dense results with scores
            if dense_results:
                logger.info("📊 TOP DENSE SCORES:")
                for i, (doc_idx, score) in enumerate(dense_results[:3]):
                    if doc_idx < len(self.documents):
                        doc_title = self.documents[doc_idx].metadata.get(
                            "title", f"doc_{doc_idx}"
                        )[:50]
                        logger.info(
                            f"   {i+1}. [{doc_idx}] {doc_title}... → {score:.4f}"
                        )
            else:
                logger.warning("⚠️ DENSE SEARCH: No results found!")

            # Step 3: Sparse keyword search (with efficiency optimization)
            logger.info(
                f"🔎 SPARSE SEARCH: BM25 keyword search for '{query}' (k={candidate_multiplier})"
            )
            ...
            if sparse_results:
                ...
                logger.warning("⚠️ SPARSE SEARCH: No results found!")
            ...
            fused_results = self.fusion_strategy.fuse_results(
                dense_results, sparse_results
            )
            logger.info(
                f"✅ FUSION RESULTS: {len(fused_results)} documents after fusion"
            )

            # Log top fused results with scores
            if fused_results:
                logger.info("📊 TOP FUSED SCORES:")
                ...
                logger.warning("⚠️ FUSION: No results after fusion!")

            # Step 4.5: Composite filtering (NEW) or semantic gap detection (LEGACY)
            if self.composite_filtering_enabled:
                # NEW: Individual document composite scoring
                filtered_results = self._calculate_composite_scores(
                    query_embedding, fused_results
                )
                if not filtered_results:
                    logger.info(
                        "Composite filtering: No documents passed quality threshold"
                    )
                    return []
                ...
                if fused_results and self.min_semantic_alignment > 0:
                    semantic_alignment = self._calculate_semantic_alignment(
                        query_embedding, fused_results[:5]
                    )
                    if semantic_alignment < self.min_semantic_alignment:
                        logger.info(
                            f"Query-document semantic alignment too low: {semantic_alignment:.3f} < {self.min_semantic_alignment}"
                        )
            ...
            if self.reranker.is_enabled() and fused_results:
                ...
                logger.info(
                    f"🔄 RERANKING: Processing {len(top_candidates)} candidates"
                )
                reranked_results = self.reranker.rerank(
                    query, candidate_documents, candidate_scores
                )
                ...
                reranked_indices = {
                    top_candidates[local_idx][0]
                    for local_idx, _ in reranked_results
                    if local_idx < len(top_candidates)
                }
                ...
                logger.info(
                    f"✅ RERANKING: Final {len(final_results)} results after reranking"
                )
            else:
                # No reranking, use fused results directly
                logger.info("⏭️ RERANKING: Skipped (reranker disabled or no results)")
            ...
            logger.info(
                f"🎯 FINAL RETRIEVAL RESULTS: {len(retrieval_results)} documents"
            )
            if retrieval_results:
                logger.info("📊 FINAL RANKING:")
                for i, result in enumerate(retrieval_results):
                    doc_title = result.document.metadata.get(
                        "title", f"doc_{result.document.content[:30]}"
                    )[:50]
                    logger.info(f"   {i+1}. {doc_title}... → {result.score:.4f}")
            else:
                logger.warning(
                    f"❌ NO RESULTS: Query '{query}' returned no relevant documents!"
                )
            ...
            logger.info(
                f"⚡ RETRIEVAL PERFORMANCE: {elapsed_time*1000:.1f}ms total, {len(retrieval_results)}/{k} results"
            )
            logger.info(
                f"🏁 RETRIEVAL COMPLETE: Query '{query}' processed successfully"
            )
    ...
    def get_configuration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sparse": {"type": "bm25", "config": self.sparse_retriever.get_stats()},
            "fusion": {
                "type": type(self.fusion_strategy)
                .__name__.lower()
                .replace("fusion", ""),
    ...
    def _consider_backend_switch(self, error: Exception) -> None:
        ...
        logger.info(
            f"Switched to backend: {self.active_backend_name} (switch count: {self.backend_switch_count})"
        )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.platform:
            try:
                component_metrics = (
                    self.platform.analytics_service.collect_component_metrics(self)
                )
                return {
    ...
    def get_capabilities(self) -> List[str]:
        ...
        if hasattr(self.vector_index, "get_capabilities"):
            capabilities.extend(
                [f"vector_{cap}" for cap in self.vector_index.get_capabilities()]
            )

        # Add sparse retriever capabilities
        if hasattr(self.sparse_retriever, "get_capabilities"):
            capabilities.extend(
                [f"sparse_{cap}" for cap in self.sparse_retriever.get_capabilities()]
            )

        # Add fusion strategy capabilities
        if hasattr(self.fusion_strategy, "get_capabilities"):
            capabilities.extend(
                [f"fusion_{cap}" for cap in self.fusion_strategy.get_capabilities()]
            )

        # Add reranker capabilities
        if hasattr(self.reranker, "get_capabilities"):
            capabilities.extend(
                [f"reranker_{cap}" for cap in self.reranker.get_capabilities()]
            )
    ...
    def get_sub_component_performance(self) -> Dict[str, Any]:
        ...
        performance = {
            ...
            "fusion_strategy": {"info": self.fusion_strategy.get_strategy_info()},
            "reranker": {
    ...
    def debug_retrieval(self, query: str, k: int = 5) -> Dict[str, Any]:
        ...
        """
        debug_info = {"query": query, "k": k, "steps": {}}
    ...
    def _calculate_composite_scores(
        self, query_embedding: np.ndarray, fused_results: List[Tuple[int, float]]
        ...
        try:
            ...
            if len(set(fusion_scores)) > 1:  # Only normalize if there's variation
                ...
                if score_range > 0:
                    normalized_fusion = [
                        (score - min_score) / score_range for score in fusion_scores
                    ]
                else:
                    normalized_fusion = [1.0] * len(
                        fusion_scores
            ...
            documents = [
                self.documents[idx] for idx in doc_indices if idx < len(self.documents)
            ]
            ...
            for i, (doc_idx, original_fusion_score) in enumerate(fused_results):
                ...
                composite_score = (
                    self.fusion_weight * normalized_fusion_score
            ...
            logger.info(
                f"COMPOSITE FILTERING - {len(fused_results)} input → {len(composite_results)} passed threshold"
            )
        ...
        try:
            ...
            for i, doc_embedding in enumerate(doc_embeddings):
                ...
                if i < 3:  # Only log first 3 docs to avoid spam
                    doc_preview = (
                        doc_texts[i][:100] + "..."
                        if len(doc_texts[i]) > 100
                        else doc_texts[i]
                    )
                    logger.debug(
                        f"Doc {i+1} similarity: {similarity:.3f} - {doc_preview}"
                    )

            # Return average similarity
            avg_similarity = np.mean(similarities) if similarities else 0.0
            logger.debug(
                f"Semantic alignment: {len(similarities)} docs, similarities={[f'{s:.3f}' for s in similarities[:5]]}, avg={avg_similarity:.3f}"
            )
