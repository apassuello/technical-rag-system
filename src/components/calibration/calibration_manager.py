...
class CalibrationManager:
    ...
    def load_test_set(self, test_set_path: Path) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.test_queries)} test queries from {test_set_path}"
            )
    ...
    def calibrate(
        ...
        if parameters_to_optimize is None:
            ...
            parameters_to_optimize = [
                ...
                "dense_weight",
                "sparse_weight",
                "score_aware_score_weight",
                "score_aware_rank_weight",
            ]

        # Generate search space
        search_space = self.parameter_registry.get_search_space(parameters_to_optimize)
        logger.info(
            f"Optimizing {len(parameters_to_optimize)} parameters with {len(list(search_space.values())[0]) if search_space else 0} combinations"
        )
        ...
        self.optimization_results[
            f"{strategy.value}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ] = result
    ...
    def calibrate_component(
        ...
        component_params = self.parameter_registry.get_parameters_for_component(
            component
        )
        if parameters:
        ...
        if test_subset:
            test_queries = [
                q for q in self.test_queries if q.get("category") == test_subset
            ]
        ...
        self.optimization_results[
            f"{component}_{strategy.value}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ] = result
    ...
    def _create_evaluation_function(
        ...
        def evaluate_parameters(parameters: Dict[str, Any]) -> float:
            ...
            try:
                ...
                config_path = self._create_config_with_parameters(
                ...
                queries_to_test = (
                    target_queries if target_queries else self.test_queries
                )
                ...
                for test_case in queries_to_test:
                    try:
                        ...
                        metrics = self.metrics_collector.start_query_collection(
                            test_case.get("test_id", f"eval_{valid_evaluations}"), query
                        )
                        ...
                        citations = (
                            result.citations if hasattr(result, "citations") else []
                        )
                        ...
                        if target_metric == "overall_accuracy":
                            ...
                            score = 1.0 - self._calculate_confidence_ece(
                                metrics
                            )  # Lower ECE is better
                            ...
                            score = metrics.validation_results.get(
                        ...
                        logger.error(
                            f"Evaluation failed for query '{test_case.get('query', 'unknown')}': {e}"
                        )
                        continue

                # Calculate average score
                final_score = (
                    total_score / valid_evaluations if valid_evaluations > 0 else 0.0
                )
    ...
    def _create_config_with_parameters(
        self, parameters: Dict[str, Any], base_config: Optional[Path] = None
    ) -> Path:
        ...
        if base_config and base_config.exists():
            ...
            config = {
                "document_processor": {
                    "type": "hybrid_pdf",
                    "config": {"chunk_size": 1024, "chunk_overlap": 128},
                },
                "embedder": {
                    "type": "modular",
                    "config": {
                        ...
                        "cache": {
                            "type": "memory",
                            "config": {"max_entries": 10000, "max_memory_mb": 512},
                        },
                    },
                },
                "retriever": {
                    ...
                    "config": {
                        ...
                        "sparse": {
                            "type": "bm25",
                            "config": {"k1": 1.2, "b": 0.75, "lowercase": True},
                        },
                        "fusion": {
                            "type": "rrf",
                            "config": {
    ...
    def _calculate_confidence_ece(self, metrics: QueryMetrics) -> float:
        ...
        actual_correctness = (
            1.0 if metrics.validation_results.get("meets_expectations", False) else 0.0
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if not self.optimization_results:
            recommendations.append(
                "No optimization results available. Run calibration first."
            )
            return recommendations

        # Find best performing configuration
        best_result = max(
            self.optimization_results.values(), key=lambda x: x.best_score
        )
        ...
        if "bm25_b" in best_params:
            ...
            if b_value < 0.3:
                recommendations.append(
                    f"BM25 b={b_value} suggests documents have varying lengths - good choice"
                )
            elif b_value > 0.7:
                recommendations.append(
                    f"BM25 b={b_value} may over-penalize longer documents"
                )
        ...
        if "rrf_k" in best_params:
            ...
            if k_value < 20:
                recommendations.append(
                    f"RRF k={k_value} provides high score discrimination"
                )
            elif k_value > 80:
    ...
    def save_optimal_configuration(
        self, output_path: Path, optimization_name: Optional[str] = None
    ) -> None:
