...
This is the production calibration metrics collector moved from the calibration
...
@dataclass
class QueryMetrics(CalibrationQueryMetrics):
    """Legacy alias for CalibrationQueryMetrics to maintain backward compatibility."""
    pass
...
class MetricsCollector(BaseMetricsCollector):
    ...
    def __init__(self) -> None:
        ...
        self.session_metadata = {
            ...
            "start_time": session_metadata_obj.start_time,
            "parameter_config": session_metadata_obj.parameter_config,
            "system_config": session_metadata_obj.system_config
        }
    ...
    def update_session_metadata(self, metadata: Dict[str, Any]) -> None:
        """Update session-level metadata (backward compatible)."""
        # Update both the legacy dict and the base class metadata object
        self.session_metadata.update(metadata)
        super().update_session_metadata(metadata)
    ...
    def _serialize_raw_data(self) -> List[Dict[str, Any]]:
        """Serialize raw query metrics data for export."""
        return [
            {
                "query_id": q.query_id,
    ...
    def export_metrics(self, output_path: Path) -> None:
        """Export collected metrics to JSON file (legacy compatible interface)."""
        try:
            export_data = {
                "session_metadata": self._serialize_session_metadata(),
                "query_metrics": self._serialize_raw_data(),
