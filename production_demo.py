...
def main():
    ...
    if rag.chunks:
        ...
        print(f"   Target range (800-2000): {sum(1 for s in sizes if 800 <= s <= 2000)}/{len(sizes)} ({sum(1 for s in sizes if 800 <= s <= 2000)/len(sizes)*100:.0f}%)")
    ...
    for method_name, method_attr in methods:
        ...
        for category, query in test_queries:
            ...
            if method_attr == "enhanced_hybrid_query":
                ...
                result = getattr(rag, method_attr)(query, top_k=3, enable_enhancement=False)
            else:
            ...
            if chunks:
                top_chunk = chunks[0]
                score_key = 'hybrid_score' if 'hybrid' in method_attr else 'similarity_score'
                ...
                # Quality assessment
                text = top_chunk['text']
                is_relevant = any(word.lower() in text.lower() for word in query.split())
                is_complete = text.strip().endswith(('.', '!', '?', ':', ';'))
                good_size = 800 <= size <= 2000
                no_trash = not any(trash in text.lower() for trash in ['creative commons', 'license'])
                ...
                print(f"      {category}: {score:.3f} | {query_time*1000:.1f}ms | Q:{quality}/4")
            else:
                method_results.append({
                    'query': query,
                    'score': 0.0,
        ...
        success_rate = sum(1 for r in method_results if r['score'] > 0) / len(method_results)
        ...
        print(f"      Summary: {avg_score:.3f} avg score | {avg_time*1000:.1f}ms avg | {success_rate*100:.0f}% success")
    
    # Performance comparison
    print(f"\n📊 Performance Comparison:")
    print(f"{'Method':<20} {'Avg Score':<12} {'Avg Time':<12} {'Quality':<10} {'Success':<10}")
    print("-" * 70)
    
    for method_name, stats in results_summary.items():
        print(f"{method_name:<20} {stats['avg_score']:<12.3f} {stats['avg_time']*1000:<12.1f} {stats['avg_quality']:<10.1f} {stats['success_rate']*100:<10.0f}%")
    ...
    if chunks:
        for i, chunk in enumerate(chunks, 1):
            ...
            if retrieval_method == 'hybrid':
                ...
                print(f"      Fusion: {dense_weight*100:.0f}% semantic + {sparse_weight*100:.0f}% keyword")
    ...
    assessment_criteria = [
        ...
        ("Query Speed", results_summary['Hybrid Search']['avg_time'] < 0.1, f"{results_summary['Hybrid Search']['avg_time']*1000:.1f}ms"),
        ("Result Quality", results_summary['Hybrid Search']['avg_quality'] >= 3.0, f"{results_summary['Hybrid Search']['avg_quality']:.1f}/4"),
        ("Success Rate", results_summary['Hybrid Search']['success_rate'] >= 0.8, f"{results_summary['Hybrid Search']['success_rate']*100:.0f}%"),
        ("Chunk Quality", sum(1 for s in sizes if 800 <= s <= 2000)/len(sizes) >= 0.8, f"{sum(1 for s in sizes if 800 <= s <= 2000)/len(sizes)*100:.0f}%")
    ]
    ...
    print(f"   Performance: {index_time:.1f}s indexing, {results_summary['Hybrid Search']['avg_time']*1000:.1f}ms queries")
    print(f"   Quality: {results_summary['Hybrid Search']['avg_quality']:.1f}/4 result quality")
    print(f"   Chunk Optimization: {sum(1 for s in sizes if 800 <= s <= 2000)/len(sizes)*100:.0f}% in target size range")
