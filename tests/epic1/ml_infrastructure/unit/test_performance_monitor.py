...
from fixtures.base_test import MLInfrastructureTestBase, PerformanceTestMixin, ConcurrencyTestMixin
        ...
        def record_latency(self, operation_name: str, latency_ms: float):
            if operation_name not in self.latency_stats:
                self.latency_stats[operation_name] = {
                    'count': 0, 'total': 0.0, 'recent': deque(maxlen=100)
                }
        ...
        def get_operation_metrics(self, operation_name: str):
            if operation_name in self.latency_stats:
                return PerformanceMetrics(
                    operation=operation_name,
                    latency_ms=self.latency_stats[operation_name]['total'] / self.latency_stats[operation_name]['count'],
                    throughput=self.throughput_stats.get(operation_name, {}).get('current', 0.0)
                )
            return None
...
class TestPerformanceMonitor(MLInfrastructureTestBase, PerformanceTestMixin, ConcurrencyTestMixin):
        ...
        if hasattr(self.monitor, 'record_quality_score'):
            ...
            if hasattr(self.monitor, 'get_quality_stats'):
                ...
                if stats and 'mean_quality' in stats:
                    ...
                    self.assertAlmostEqual(stats['mean_quality'], expected_mean, places=2)
        ...
        if hasattr(self.monitor, 'record_latency'):
            ...
            for i in range(20):
                ...
                if hasattr(self.monitor, 'record_latency_with_timestamp'):
                    self.monitor.record_latency_with_timestamp(operation_name, latency, timestamp)
                else:
                    self.monitor.record_latency(operation_name, latency)
            ...
            if hasattr(self.monitor, 'get_metrics_for_time_window'):
                ...
                if recent_metrics and extended_metrics:
                    ...
                    if 'count' in recent_metrics and 'count' in extended_metrics:
                        self.assertLessEqual(recent_metrics['count'], extended_metrics['count'])
            ...
            if hasattr(self.monitor, 'get_performance_trend'):
                ...
                if trend:
                    ...
                    if 'direction' in trend:
                        self.assertIn(trend['direction'], ['degrading', 'worsening', 'increasing'])
...
class TestPerformanceMonitorPerformance(MLInfrastructureTestBase, PerformanceTestMixin):
    ...
    def test_monitoring_overhead(self):
        ...
        if hasattr(self.monitor, 'record_latency'):
            ...
            results = self.benchmark_operation(record_latency_op, iterations=1000, warmup=100)
