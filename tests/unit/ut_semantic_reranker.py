...
@pytest.mark.xfail(
    reason="RetrievalResult rejects negative scores but NeuralReranker can produce them",
    strict=True,
)
def test_retrieval_result_accepts_negative_reranker_scores():
    """NeuralReranker/CrossEncoder can return negative relevance scores.

    RetrievalResult.__post_init__ raises ValueError for score < 0,
    making the reranker path unusable without clamping.
    """
    from src.core.interfaces import Document, RetrievalResult

    doc = Document(content="test", metadata={})
    # CrossEncoder scores are unbounded — negative means "not relevant"
    RetrievalResult(document=doc, score=-0.5, retrieval_method="reranker")
