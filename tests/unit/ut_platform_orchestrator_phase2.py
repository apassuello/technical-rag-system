...
class TestPlatformOrchestratorPhase2:
    ...
    @patch('src.core.platform_orchestrator.ComponentFactory')
    def test_unified_architecture_initialization(self, mock_factory, unified_config, temp_config_file):
        ...
        # Verify factory calls (no vector store created)
        mock_factory.create_processor.assert_called_once()
        mock_factory.create_embedder.assert_called_once()
        mock_factory.create_vector_store.assert_not_called()
        mock_factory.create_retriever.assert_called_once()
        mock_factory.create_generator.assert_called_once()
    ...
    @patch('src.core.platform_orchestrator.ComponentFactory')
    def test_unified_document_processing(self, mock_factory, unified_config, temp_config_file):
    ...
    @patch('src.core.platform_orchestrator.ComponentFactory')
    def test_query_processing_compatibility(self, mock_factory, unified_config, temp_config_file):
    ...
    @patch('src.core.platform_orchestrator.ComponentFactory')
    def test_configuration_validation(self, mock_factory, temp_config_file):
        ...
        # Mock factory to raise error for invalid config
        mock_factory.create_retriever.side_effect = TypeError("Missing required argument")
        mock_factory.create_processor.return_value = Mock()
        mock_factory.create_embedder.return_value = Mock()
        mock_factory.create_generator.return_value = Mock()
        mock_factory.create_query_processor.return_value = Mock()
    ...
    @patch('src.core.platform_orchestrator.ComponentFactory')
    def test_component_health_monitoring(self, mock_factory, unified_config, temp_config_file):
        ...
        mock_factory.create_processor.return_value = mock_components['document_processor']
        ...
        mock_factory.create_generator.return_value = mock_components['answer_generator']
    ...
    def test_architecture_detection_logging(self, unified_config, temp_config_file, caplog):
        ...
        with patch('src.core.platform_orchestrator.ComponentFactory') as mock_factory:
            ...
            # Check that Phase 3 initialization is logged
            assert "Phase 3: Unified retriever initialized" in caplog.text
