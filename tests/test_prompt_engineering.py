...
class PromptEngineeringTester:
    ...
    def test_baseline_prompts(self) -> Dict[str, Any]:
        ...
        for query_name, query in self.test_queries.items():
            ...
            results[query_name] = {
                ...
                "has_few_shot": "few-shot" in prompt["user"].lower()
                or "examples" in prompt["user"].lower(),
            }
    ...
    def test_few_shot_prompts(self) -> Dict[str, Any]:
        ...
        for query_name, query in self.test_queries.items():
            ...
            results[query_name] = {
                ...
                "few_shot_examples": (
                    len(template.few_shot_examples) if template.few_shot_examples else 0
                ),
            }
        ...
        for query_name, query in self.test_queries.items():
            ...
            for chunk in chunks:
                if isinstance(chunk, dict):
                    ...
                    formatted_chunks.append(
                        {"content": chunk, "metadata": {}, "confidence": 0.5}
                    )
                else:
                    formatted_chunks.append(
                        {"content": str(chunk), "metadata": {}, "confidence": 0.5}
                    )

            # Generate adaptive configuration
            config = self.adaptive_engine.generate_adaptive_config(
                query=query, context_chunks=formatted_chunks, prefer_speed=False
            )

            # Create adaptive prompt
            adaptive_prompt = self.adaptive_engine.create_adaptive_prompt(
                query=query, context_chunks=formatted_chunks, config=config
            )
    ...
    def test_chain_of_thought_prompts(self) -> Dict[str, Any]:
        ...
        for query_name, query in complex_queries.items():
            ...
            print(
                f"  - Has reasoning steps: {results[query_name]['has_reasoning_steps']}"
            )
            print(
                f"  - Reasoning framework: {results[query_name]['reasoning_framework']}"
            )
    ...
    def run_performance_comparison(self) -> Dict[str, Any]:
        ...
        for prompt_type, results in comparison.items():
            if results:
                avg_prompt_length = sum(
                    r["prompt_length"] for r in results.values()
                ) / len(results)
                avg_generation_time = sum(
                    r["prompt_generation_time"] for r in results.values()
                ) / len(results)
        ...
        return {"detailed_results": comparison, "summary": summary}

    def save_results(
        self, results: Dict[str, Any], filename: str = "prompt_test_results.json"
    ):
